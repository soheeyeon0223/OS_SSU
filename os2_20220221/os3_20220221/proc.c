...
prioritymanager(struct proc *p) {
	...
	p->runcount[idx]++; // 현재 우선순위에서 실행 횟수 증가시킴
	
	// 우선순위 증가 조건
	if(p->runcount[idx] >= 100) {
		p->runcount[idx] = 0; // 100번 도달시 runcount 초기화
...
scheduler(void)
{
  ...
  for(int i = 1; i <= 10; i++) {
	  ...
	  // 우선 순위가 높을수록 더 많이 실행
	  for(int j = 10; j >= i; j--) { // 상기 주석을 만족하기 위해 반복
		  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			  if(p->state != RUNNABLE || p->priority != i) // 우선순위 i와 일치하는 실행 가능한 프로세스만 선택
				  continue;
      ...
      prioritymanager(p); // 우선 순위를 관리하기 위해 추가
